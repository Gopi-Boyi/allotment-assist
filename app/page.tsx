...
const documents = [
  ...
  { category: 'Other Requirements', title: 'Joining Letter', note: 'Original + 2 photocopies', icon: FileText },
  { category: 'Other Requirements', title: 'Allotment Order', note: 'Original + 2 photocopies', icon: ScrollText },
...
export default function Page() {
  ...
  const [bookingDetails, setBookingDetails] = useState({ name: '', mobile: '', email: '', address: '', accommodation: 'No', hostelType: '' })
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      {activePanel === 'home' && <>
        ...
        <section className="shell feature-grid">
          ...
          <button className="feature-card feature-green" onClick={() => setActivePanel('navigation')}><span className="icon-box"><Navigation /></span><div><p className="card-label">03 · ON CAMPUS</p><h4>Find your way</h4><p>Block, floor, room and the person you need to meet.</p></div><ArrowRight className="card-arrow" /></button><button className="feature-card feature-white" onClick={() => setActivePanel('university')}><span className="icon-box"><Building2 /></span><div><p className="card-label">04 · OFFICIAL DETAILS</p><h4>About Aditya University</h4><p>Address, MCA eligibility, fee guidance and official contacts.</p></div><ArrowRight className="card-arrow" /></button><button className="feature-card feature-purple" onClick={() => setActivePanel('navigation')}><span className="icon-box"><Shirt /></span><div><p className="card-label">02 · AFTER ALLOTMENT</p><h4>Buy your uniform</h4><p>Purchase your official uniform at Cotton Bhavan cash counter after your reporting slot is confirmed.</p><p className="card-meta"><strong>₹1,800</strong> · Cash payment only</p></div><ArrowRight className="card-arrow" /></button>
        </section>
      </>}
      ...
      {activePanel === 'navigation' && <section className="shell panel-page"><div className="panel-heading"><div><p className="section-label">ARRIVE WITH CONFIDENCE</p><h2>Campus navigation</h2><p>Follow this route from the main gate to your MCA verification desk.</p></div><button className="outline-btn" onClick={openSlot}>Book a slot <ArrowRight size={16} /></button></div><p className="map-microcopy"><MapPin size={15} /> Estimated walk time: 5–7 minutes from the Main Gate</p><div className="navigation-layout"><div className="map-column"><div className="campus-map location-map"><CampusJourneyMap /><div className="location-map-label"><MapPin size={16} /><span><strong>Campus journey</strong><small>Main Gate → MCA Block → Cotton Bhavan</small></span></div></div></div><div className="route-details"><div className="route-timeline"><div className="route-step"><span className="step-number">1</span><div><p className="card-label">FROM THE MAIN GATE</p><h4><Navigation size={17} /> Walk to MCA Block</h4><p>Follow the central pathway past the Admin Block. It takes about 4 minutes.</p><a className="step-map-link" href="https://www.google.com/maps/search/?api=1&query=17.0878598,82.0667294" target="_blank" rel="noreferrer"><MapPin size={14} /> Open in Google Maps</a></div></div><div className="route-step"><span className="step-number">2</span><div><p className="card-label">AFTER ALLOTMENT</p><h4><Shirt size={17} /> Cotton Bhavan · Cash Counter</h4><p><strong>₹1,800</strong> · Cash payment and, UPI/card accepted. Open <strong>9:00 AM – 4:00 PM</strong>.<br /><span className="coordinates">17.0879784, 82.0669404</span></p><a className="step-map-link" href="https://www.google.com/maps/place/Cotton+Bhavan/@17.0879719,82.066973,498m/data=!3m1!1e3!4m6!3m5!1s0x3a3782e1e3fdd3f5:0x745945becc24b317!8m2!3d17.0879784!4d82.0669404!16s%2Fg%2F12lt2k4tt" target="_blank" rel="noreferrer"><MapPin size={14} /> Open in Google Maps</a></div></div></div></div></div></section>}
